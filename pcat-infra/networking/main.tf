...
resource "aws_acm_certificate" "cert_mh370api" {
  domain_name       = "mh370-api.${local.dns_zone}"
  validation_method = "DNS"
}
...
resource "aws_route53_record" "cert_mh370api_validation" {
  name    = aws_acm_certificate.cert_mh370api.domain_validation_options.0.resource_record_name
  type    = aws_acm_certificate.cert_mh370api.domain_validation_options.0.resource_record_type
  zone_id = data.aws_route53_zone.zone.id
  records = [aws_acm_certificate.cert_mh370api.domain_validation_options.0.resource_record_value]
  ttl     = 60
}
...
resource "aws_acm_certificate_validation" "cert_mh370api" {
  certificate_arn         = aws_acm_certificate.cert_mh370api.arn
  validation_record_fqdns = [aws_route53_record.cert_mh370api_validation.fqdn]
}
